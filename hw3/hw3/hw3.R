...
link.prediction <- function(igraph.object, cutoff.percentage){
        ...
        set.seed(1)
        #sample edges
        sample.edges <- sample(1:length(E(igraph.object)), round(length(E(igraph.object))*(cutoff.percentage/100)))
        ...
        answer <- cbind(vertices[1:(length(vertices)/2)], vertices[((length(vertices)/2)+1):length(vertices)], round(pred.deleted.edges.prob,4))
